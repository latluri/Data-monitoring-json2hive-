...
def json_extract_keys(g1):
    primary=json.loads(g1["entitystring"])
    secondary=json.loads(json.loads(g1["entitystring"])["entitydata"])
    primary["message__id"]=primary.pop("messageid")
    final_dict=dict(primary.items()+secondary.items())
    return(final_dict)



def json_extract_nested_keys(g1):
    entitydata=json.loads(json.loads(g1["entitystring"])["entitydata"])
    keys_=[]
    for k21 in entitydata.keys():
        try:
            keys_1=json.loads(entitydata[k21]).keys()
            keys_.extend(keys_1)
        except:
            pass
    return(keys_)
...
for k in tables_gcct_except_bad_rec:
            ...
            g1_1=v1.count()
            g1=g1+g1_1
            v2=v1.select("entitystring").rdd.map(lambda g1: json_extract_nested_keys(g1)).filter(lambda x: len(x) > 0).flatMap(lambda g2: [k.lower() for k in g2]).collect()
	    v3=set(v2)
            if len(v3) >0:
                print "\\nThe table "+str(k)+" seem to have nested data " +str(",".join(v3))+"\\n\\n"
	    ...
	    y1=sqlContext.table(k).groupby("shakey").count().filter("count >1")
            if y1.count() > 0:
                ...
                y2_1=y1.select("shakey").distinct().collect()
		y2=[row["shakey"] for row in y2_1]
		print("\\nTable: "+str(k)+" has "+str(y1.count())+" duplicate(s). The following are the shakeys: "+ str(",".join(y2))+"\\n\\n")
		duplicates='Duplicate(s) are observed'
            col_list=[col.lower() for col in v1.columns]
            v4_1=v1.select("entitystring").rdd.map(lambda g1: json_extract_keys(g1)).filter(lambda x: len(x) > 0).flatMap(lambda g2: [k.lower() for k in g2.keys()]).collect()
	    v4=set(v4_1).difference(set(col_list))
            v5=set(v4).difference(set(ignore_list))
            if len(v5) >0:
                print("\\nTable: "+str(k)+" seem to get new Field(s) "+str(",".join(v5))+"\\n\\n")
...
if g3.count() > 0:
     ...
     g3_1=g3.collect()
     g3_2=[ str(i["datatype"])+"  "+str(i["count"]) for i in g3_1]
     print(g3_2)
...
if g4.count() > 0:
     ...
     g4_1=g4.collect()
     g4_2=[ str(i["datatype"])+"  "+str(i["count"]) for i in g4_1]
     print(g4_2)
